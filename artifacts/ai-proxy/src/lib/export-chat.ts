...
// ─────────────────────────────────────────────────────────────────────────────
// Word (.docx) export — Python backend (python-docx, richer styling)
// Falls back automatically to client-side if the backend is unreachable.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Build the API base URL the same way other fetches in the app do.
 * Works under Replit's path-based proxy and on custom domains.
 */
...
/**
 * Serialise messages for the backend — timestamps are sent as ISO strings.
 */
function serializeMessages(messages: ExportMessage[]) {
...
/**
 * Download a Word document generated by the Python backend (python-docx).
 * Falls back to the client-side docx package on any network/backend error.
 */
export async function exportChatAsDocxBackend(opts: ExportOptions): Promise<void> {
  ...
  try {
    ...
    // Derive filename from Content-Disposition if available, otherwise build one
    const disposition = res.headers.get("content-disposition") ?? "";
    const fileMatch   = disposition.match(/filename="?([^";]+)"?/);
    const slug  = title.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
    const fname = fileMatch?.[1] ?? `${slug}-${new Date().toISOString().slice(0, 10)}.docx`;
    ...
    console.warn("[exportChatAsDocxBackend] Backend unavailable, falling back to client-side:", err);
    // Fallback — client-side docx package (already imported above)
    await exportChatAsDocx(opts);
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// Save to workspace — writes the conversation as a Markdown file
// ─────────────────────────────────────────────────────────────────────────────

export interface SaveToWorkspaceOptions extends ExportOptions {
  /** Workspace-relative path where the file is written (default: "chat-export-YYYY-MM-DD.md") */
  workspacePath?: string;
}

/**
 * Convert the conversation to readable Markdown and write it to the current
 * workspace via the `/api/files/write` endpoint.
 * Returns the path the file was written to, or throws on error.
 */
export async function saveChatToWorkspace(opts: SaveToWorkspaceOptions): Promise<string> {
  const { title = "Vesper Chat Export", workspaceName, workspacePath, messages } = opts;
  const now = new Date();
  const slug = title.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
  const path = workspacePath ?? `${slug}-${now.toISOString().slice(0, 10)}.md`;
  ...
  if (!res.ok) {
    ...
    throw new Error((err as any).error ?? `Write failed (${res.status})`);
  }
