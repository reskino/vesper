...
*   **UI/UX**:
    ...
    *   **Live Preview Panel**: Embedded browser (iframe) that proxies any localhost port through the API server's `/api/port-proxy/:port/` endpoint. Accessible via the Globe icon in the activity bar. Features: URL address bar, back/forward/refresh navigation, responsive viewport switcher (desktop/tablet/mobile), maximize/minimize, auto-open when a server URL is detected in run output. Quick-launch buttons for common ports (3000, 5000, 8000, 8080).
